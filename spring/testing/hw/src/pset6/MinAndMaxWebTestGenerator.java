...
public class MinAndMaxWebTestGenerator {
    ...
    String testsuite() {
        ...
        String[] xvals = { "infinity", "-3", "0", "7" };
        String[] yvals = { "infinity", "-3", "0", "7" };
        String[] zvals = { "infinity", "-3", "0", "7" };
        boolean[] clickvals = { true, false };

        // true = max. false = min
        boolean[] radiovals = { true, false };

        // loop over the input partition space. 
        int id = 0; // test id;
        for (boolean radio : radiovals)
            for (boolean click : clickvals)
                for (String x : xvals)
                    for (String y : yvals)
                        for (String z : zvals)
                            sb.append(testMethod(id++, x, y, z, click, radio) + "\n\n");
        // close function
        sb.append("}\n");
        return sb.toString();
    }
     ...
     */
    String testMethod(int id, String x, String y, String z, boolean click, boolean radio) {
        StringBuilder builder = new StringBuilder();
        ...
        String file = "file:////home/ronny/school/testing/hw/minandmax.html";
        builder.append(String.format("// test: <%s, %s, %s, %s, %s> \n", x, y, z, (click ? "click" : "noclick"), (radio ? "max" : "min")));
        ...
        String radioId = radio ? "max" : "min";  // check the radio button.
        builder.append(String.format("driver.findElement(By.id(\"%s\")).click(); // computeButton click \n", radioId));
        ...
        if (click) { // verify test with assertion.
            ...
            String extrema = "\"Please enter integer values only!\"";
            if (!x.equals("infinity") && !y.equals("infinity") && !z.equals("infinity") && !radio) {
                ...
                extrema = String.format("\"min(%s, %s, %s) = %d\"", x, y, z, value);
            } else if (!x.equals("infinity") && !y.equals("infinity") && !z.equals("infinity") && radio) {
                Integer value = (Integer) Math.max(Math.max(Integer.parseInt(x), Integer.parseInt(y)), Math.max(Integer.parseInt(x), Integer.parseInt(z)));
                extrema = String.format("\"max(%s, %s, %s) = %d\"", x, y, z, value);
            }
